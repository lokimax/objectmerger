...
public final class MvelSandbox {
    ...
    public static void validateExpression(String expression) {
        ...
        rejectTypeReference(expression);
    }
    ...
    private static void rejectTypeReference(String expression) {
        if (MVEL_TYPE_REFERENCE_PATTERN.matcher(expression).find()) {
            throw new SecurityException(
                    "MVEL expression blocked by sandbox: 'T(...)' type reference is not allowed. Expression: "
                            + expression);
        }
    }
