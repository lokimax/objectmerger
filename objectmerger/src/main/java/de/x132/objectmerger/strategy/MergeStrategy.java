 ...
 *
 * <p>
 * Values are merged by strategies loaded via Java's
 * {@link java.util.ServiceLoader} mechanism.
 * To implement a custom strategy:
 *
 * <ol>
 * <li>Implement this interface.
 * <li>Create a specific {@link FieldDefinition} subclass if custom
 ...
 * @param <C> The specific type of {@link FieldDefinition} required by this
 *            strategy.
 */
public interface MergeStrategy<T, C extends FieldDefinition<T>> {
  ...
  /**
   * Returns the unique name of this strategy. This name is used in the
   * configuration to refer to
   * this strategy.
   *
   ...
   *
   * <p>
   * <strong>Contract:</strong> This method must NOT return {@code null}. It must
   * return the
   * specific subclass of {@link FieldDefinition} that this strategy expects. This
   * is used by the
   * runtime to validate that the configuration provided in the merge definition
   * matches the
